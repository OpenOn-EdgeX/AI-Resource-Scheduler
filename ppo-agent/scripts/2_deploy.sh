...
kubectl rollout status daemonset/ppo-agent -n edge-system --timeout=120s || true
...
echo ""
echo "Check status:"
echo "  kubectl get pods -n edge-system -l app=ppo-agent"
echo "  kubectl logs -n edge-system -l app=ppo-agent"
